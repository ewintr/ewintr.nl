...
_Note: If you're the type of reader that likes code better than words, skip this explanation and go directly to the test/doc folder in [this repository](https://codeberg.org/ewintr/go-kit) that contains a complete working example of everything discussed below._
...
_Note: the `test.Equals` are part of the small test package in [this go-kit](https://codeberg.org/ewintr/go-kit). The discussed http mock also belongs to that package and together they form a minimal, but sufficient set of test helpers. But if you prefer, you can of course combine this with populair libraries like [testify](https://pkg.go.dev/github.com/stretchr/testify/assert?tab=doc)._
...
It would be too much to discuss all details of `MockAssertion` here. If you want, you can inspect the code in `test/httpmock.go` in the mentioned [go-kit repository](https://codeberg.org/ewintr/go-kit). For now, let's keep it at these observations:
